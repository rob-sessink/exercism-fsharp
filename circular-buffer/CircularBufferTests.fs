...
[<Fact>]
let ``Iterate over empty buffer is not possible`` () =
    let buffer1 = mkCircularBuffer 3
    (fun () -> iter (fun e -> printfn "%i" e) buffer1 |> ignore) |> should throw typeof<Exception>

[<Fact>]
let ``Iterate over buffer, invoking func`` () =
    let buffer1 = mkCircularBuffer 3
    let buffer2 = write 1 buffer1
    let buffer3 = write 2 buffer2
...
let ``Peeking an empty buffer is not possible`` () =
    let buffer1 = mkCircularBuffer 3
    (fun () -> peek buffer1 |> ignore) |> should throw typeof<Exception>
...
let ``Peek a buffer`` () =
    ...
    let buffer2 = write 1 buffer1
    let buffer3 = write 2 buffer2
    let val1 = peek buffer3
    val1 |> should equal 1

[<Fact>]
let ``length of an empty buffer is not possible`` () =
    let buffer1 = mkCircularBuffer 3
    (fun () -> length buffer1 |> ignore) |> should throw typeof<Exception>

[<Fact>]
let ``length of a full buffer is size`` () =
    let buffer1 = mkCircularBuffer 2
    let buffer2 = write 1 buffer1
    let buffer3 = write 2 buffer2
    let length = length buffer3
    length |> should equal 2

[<Fact>]
let ``length of half buffer is half size`` () =
    let buffer1 = mkCircularBuffer 2
    let buffer2 = write 1 buffer1
    let length = length buffer2
    length |> should equal 1

[<Fact>]
let ``length of overwritten buffer is size`` () =
    let buffer1 = mkCircularBuffer 2
    let buffer2 = write 1 buffer1
    let buffer3 = write 2 buffer2
    let buffer4 = forceWrite 3 buffer3
